...
class TCDScheduler(SchedulerMixin, ConfigMixin):
    ...
    def set_timesteps(
        ...
        if self.model_coeff == None:
            self.model_coeff = torch.ones((num_inference_steps,))
        if self.sample_coeff == None:
            self.sample_coeff = torch.ones((num_inference_steps,))
        self._step_index = None
        self._begin_index = None
    ...
    def step(
        ...
        # alpha_prod_s = alpha_prod_s *  self.alpha_prod_s_coeff
        beta_prod_s = 1 - alpha_prod_s
        # beta_prod_s = beta_prod_s * self.beta_prod_s_coeff
        # beta_prod_s *= 0.1
        # 3. Compute the predicted noised sample x_s based on the model parameterization
        model_output = model_output * self.model_coeff[self.step_index]
        sample = sample * self.sample_coeff[self.step_index]
        if self.config.prediction_type == "epsilon":  # noise-prediction
            ...
            pred_noised_sample = alpha_prod_s.sqrt() * model_output + beta_prod_s.sqrt() * pred_epsilon
        ...
        if eta > 0:
            if self.step_index != self.num_inference_steps - 1:
                ...
                stochastic_noise_coeff = 1 - alpha_prod_t_prev / alpha_prod_s
                if ptqd_k is not None:
                    uncorrelated_noise_coeff = alpha_prod_t_prev.sqrt() / (alpha_prod_s.sqrt() * (1+ptqd_k[self.step_index]))
                    uncorrelated_noise_coeff *= (beta_prod_s.sqrt() - (alpha_prod_s.sqrt() * beta_prod_t.sqrt())/alpha_prod_t.sqrt())
                    uncorrelated_noise_coeff  = (uncorrelated_noise_coeff * ptqd_sigma[self.step_index])**2
                    if stochastic_noise_coeff >= uncorrelated_noise_coeff:
                        print("applying correction")
                        stochastic_noise_coeff -= uncorrelated_noise_coeff
                    # numerator = beta_prod_s.sqrt() - ((alpha_prod_s.sqrt() * beta_prod_t.sqrt())/ alpha_prod_t.sqrt()) 
                    
                    # temp_alpha_prod_t_prev = alpha_prod_s - alpha_prod_t_prev * (1 + (numerator/ (1+ptqd_k[self.step_index]))**2)
                    # print(temp_alpha_prod_t_prev)
                    # if temp_alpha_prod_t_prev > 0:
                    #     print("using modified alpha_prod_t_prev")
                    #     alpha_prod_t_prev = temp_alpha_prod_t_prev
                prev_sample = (alpha_prod_t_prev / alpha_prod_s).sqrt() * pred_noised_sample + stochastic_noise_coeff.sqrt() * noise
            else:
